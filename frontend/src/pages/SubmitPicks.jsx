  ...
  const renderTeam = (team, isP4, isUsed, winProb, opponent, canWin, canUpset, thursdayLocked) => {
    ...
    return (
      <div style={{ flex: 1, display: 'flex', flexDirection: 'column', gap: 4 }}>
        ...
        {!isLocked && !isUsed && (canWin || canUpset) && (() => {
          // Each team has exactly one pick type: win (P4vP4) or upset (P4vNonP4).
          const isUpset = canUpset && !canWin;
          const isPicked = isUpset ? isPickedUpset : isPickedWin;
          const pts = isUpset ? 2 : 1;
          const label = isUpset ? '⚡ Upset' : 'Win';
          return (
            <button
              onClick={() => onPick(
                team,
                isUpset ? 'upset_loss' : 'win_vs_power4',
                opponent,
                isUpset ? upsetProb : winProb
              )}
              className="btn btn-sm"
              style={{
                width: '100%', fontSize: 14, padding: '8px 6px', marginTop: 2,
                background: isPicked
                  ? (isUpset ? 'var(--amber-pencil)' : 'var(--ink)')
                  : 'transparent',
                border: `1.5px solid ${isPicked
                  ? (isUpset ? 'var(--amber-pencil)' : 'var(--ink)')
                  : (isUpset ? 'var(--amber-pencil)' : 'var(--rule-dark)')}`,
                color: isPicked
                  ? 'var(--paper)'
                  : (isUpset ? 'var(--amber-pencil)' : 'var(--ink)'),
            ...
            >
              {isPicked ? `✓ ${label} · ${pts}pt` : `${label} · ${pts} pt`}
            </button>
          );
        })()}
      </div>
